...
from .utils import load_yaml, find_node_by_path
...
def test_invalid_directory_path(run_mapper, capsys):
    ...
    assert not run_mapper([dir_name]) # run_mapper вернет False при SystemExit != 0
    captured = capsys.readouterr()

    # ---> ИЗМЕНЕНИЕ: Проверяем актуальное сообщение об ошибке <---
    # cli.py теперь выводит "Error: The directory '...' does not exist." в stderr
    # или "Error: The path '...' is not a valid directory." если путь существует, но не директория
    # Проверим наличие ключевых частей сообщения в stderr
    ...
    # Можно убрать проверку stdout, если print идет только в stderr при ошибке
    # assert "Error: The path" not in captured.out
...
# --- Тесты на права доступа (могут быть нестабильны/требовать прав) ---

@pytest.fixture
def set_perms(request):
    # ... (код фикстуры set_perms без изменений) ...
    original_perms = {}
    paths_changed = []
...
def test_output_path_is_directory(temp_project, run_mapper, caplog):
    ...
    output_should_be_file.mkdir() # Создаем директорию с таким именем

    # Ожидаем ошибку IOError (или подкласс) при попытке открыть директорию для записи
    with caplog.at_level(logging.ERROR):
        # run_mapper должен вернуть True, если ошибка поймана и залогирована в writer.py
        # Но сам main может упасть из-за raise в writer.py, тогда run_mapper вернет False
        # Проверяем лог в любом случае
    ...
    # Содержимое директории не должно измениться
    assert output_should_be_file.is_dir()
    assert not list(output_should_be_file.iterdir()) # Должна остаться пустой
