...
import logging
import sys
from .utils import load_yaml, get_all_files_in_tree, find_node_by_path
...
def test_empty_directory(temp_project, run_mapper):
    ...
    assert "children" not in result or not result["children"] # Не должно быть детей
...
def test_filenames_with_special_yaml_chars(temp_project, run_mapper):
    ...
    # Создаем файлы с именами, которые могут вызвать проблемы при ручной записи YAML
    # Используем символы, допустимые в именах файлов на большинстве систем
    # ---> ИЗМЕНЕНИЕ: Удален файл с ':' <---
    # (temp_project / "file_with:colon.txt").touch()
    ...
    # Проверяем, что файлы были включены (если ОС позволила их создать)
    # if (temp_project / "file_with:colon.txt").exists(): # Удалено
    #     assert "file_with:colon.txt" in all_files     # Удалено
