...
def test_empty_and_invalid_ignores(temp_project, run_mapper):
    ...
    assert run_mapper([".", "-i", str(temp_project / "empty.ignore"), "-o", "out_custom_empty.yaml"])
    assert run_mapper([".", "-i", str(temp_project / "invalid.ignore"), "-o", "out_invalid.yaml"])
...
def test_ignore_double_star(temp_project, run_mapper, caplog): # Добавили caplog
    """Тест: игнорирование с помощью '**'."""
    # ---> ИЗМЕНЕНИЕ: Включаем DEBUG логирование для анализа <---
    ...
    # ---> ИЗМЕНЕНИЕ: Используем стандартный assert <---
...
def test_ignore_output_file_itself(temp_project, run_mapper):
    ...
    # Создаем директорию заранее в тесте
...
def test_no_default_ignores_flag(temp_project, run_mapper):
    ...
    (temp_project / ".gitignore").write_text("*.pyc\n")
    # Используем другое имя для .treemapperignore чтобы он не влиял на другие тесты
    ...
    # Передаем фиктивный кастомный ignore file, чтобы проверить, что только он используется
    custom_ignore = temp_project / "custom_empty.ignore"
    custom_ignore.touch()
    assert run_mapper([".", "--no-default-ignores", "-i", str(custom_ignore) ,"-o", str(output_path)])
    result = load_yaml(output_path)
    all_files = get_all_files_in_tree(result)
    ...
    assert ".gitignore" in all_files
    assert ".treemapperignore" in all_files # Файл из фикстуры temp_project
    assert treemapper_ignore.name in all_files # Файл .tmignore_fortest
    assert custom_ignore.name in all_files # Файл custom_empty.ignore
    assert output_path.name not in all_files, f"Output file {output_path.name} should still be ignored even with --no-default-ignores"
...
def test_unreadable_ignore_file(temp_project, run_mapper, set_perms, caplog):
    ...
    ignore_file.write_text(".git/\n") # Неважно что, главное создать
    set_perms(ignore_file, 0o000) # Убираем права

    # Ожидаем предупреждение в логах, но успешное выполнение программы
...
def test_bad_encoding_ignore_file(temp_project, run_mapper, caplog):
    ...
    ignore_file = temp_project / ".treemapperignore"
    try:
        # Пытаемся записать в CP1251
        ignore_file.write_text(".git/\nпапка_игнор/\n", encoding='cp1251')
    except LookupError:
        pytest.skip("CP1251 codec not found")

    # Ожидаем предупреждение о невозможности декодировать, но успешное выполнение
    with caplog.at_level(logging.WARNING):
         assert run_mapper(["."])
    ...
    # Проверим, что программа отработала без учета этого файла (т.е. папка не игнорируется)
    result = load_yaml(temp_project / "directory_tree.yaml")
    # Добавим папку, которая должна была игнорироваться файлом
    (temp_project / "папка_игнор").mkdir()
    # Перезапустим маппер, чтобы он увидел новую папку
    assert run_mapper(["."])
    result = load_yaml(temp_project / "directory_tree.yaml")
    all_files = get_all_files_in_tree(result)
    assert "папка_игнор" in all_files # Папка не должна быть проигнорирована
