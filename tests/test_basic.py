...
import logging
import shutil
from .utils import load_yaml, get_all_files_in_tree, find_node_by_path, make_hashable
...
def normalize_tree(tree):
    ...
    # Используется теперь только для отладки или специфичных сравнений
...
def test_file_content_encoding(temp_project, run_mapper):
    ...
    assert empty_node is not None and empty_node.get("content") == empty_content_orig # Пустая строка остается пустой
...
def test_absolute_relative_paths(temp_project, run_mapper):
    ...
    # Check src vs src_node_in_root children using sets of hashable items
    output_files_to_ignore_src = {output_path_rel_src.name}
    src_children_set = {make_hashable(child) for child in src_result.get("children", [])
                       if child.get("name") not in output_files_to_ignore_src}
    ...
    # Check abs vs root children using sets of hashable items, filtering all output files
    output_files_to_ignore_root = {output_path_abs.name, output_path_rel_root.name, output_path_rel_src.name}
    abs_children_set = {make_hashable(child) for child in abs_result.get("children", [])
                        if child.get("name") not in output_files_to_ignore_root}
    root_children_set = {make_hashable(child) for child in root_result.get("children", [])
                         if child.get("name") not in output_files_to_ignore_root}
...
def test_unicode_content_and_encoding_errors(temp_project, run_mapper, caplog):
    ...
    # Ожидаем, что CP1251 вызовет UnicodeDecodeError и будет заглушка
    assert cp1251_node is not None, "'cp1251.txt' not found"
    # ---> ИЗМЕНЕНИЕ: Добавляем \n к ожидаемому контенту-заглушке <---
    assert cp1251_node.get("content") == "<unreadable content: not utf-8>\n"
    assert any(
        "Cannot decode cp1251.txt as UTF-8" in record.message
    ...
    # Ожидаем, что binary.bin тоже вызовет ошибку и будет заглушка
    assert binary_node is not None, "'binary.bin' not found"
    # ---> ИЗМЕНЕНИЕ: Добавляем \n к ожидаемому контенту-заглушке <---
    # (Даже если чтение не удалось совсем, в node["content"] должна быть строка-ошибка)
    assert isinstance(binary_node.get("content"), str) # Должно быть строкой
    assert binary_node.get("content", "").startswith("<unreadable content")
    if binary_node.get("content") != "<unreadable content>": # Add newline if it's not the exact base error string
         assert binary_node.get("content", "").endswith("\n")

    # Проверяем лог (ожидаем Warning или Error)
    assert any(
        ("Cannot decode binary.bin as UTF-8" in record.message and record.levelno >= logging.WARNING) or \
        ("Could not read binary.bin" in record.message and record.levelno >= logging.ERROR) or \
        ("Unexpected error reading binary.bin" in record.message and record.levelno >= logging.ERROR) or \
        ("Removed NULL bytes from content of binary.bin" in record.message and record.levelno >= logging.WARNING)
        for record in caplog.records
    ), "Expected WARNING/ERROR log message about reading/decoding failure not found for binary.bin"

def test_unicode_content_and_encoding_errors(temp_project, run_mapper, caplog):
    """Тест: содержимое в UTF-8, не-UTF8 (CP1251), бинарное, с NULL.""" # Добавлено в описание
    utf8_content = "привет мир"
    try:
        cp1251_content_bytes = "тест".encode('cp1251')
    except LookupError:
        pytest.skip("CP1251 codec not found, skipping test")
    binary_content = b'\x80\x9F\xFF' # Убрали \x00 отсюда, чтобы он не ломал загрузку YAML
    # ---> НАЧАЛО ДОБАВЛЕНИЯ <---
    ...
    (temp_project / "binary.bin").write_bytes(binary_content)
    # ---> НАЧАЛО ДОБАВЛЕНИЯ <---
    (temp_project / "null_byte.txt").write_bytes(content_with_null)
    # ---> КОНЕЦ ДОБАВЛЕНИЯ <---

    output_path = temp_project / "encodings_output.yaml"
    with caplog.at_level(logging.WARNING):
        assert run_mapper([".", "-o", str(output_path)])
    # Теперь load_yaml не должен падать из-за \x00, т.к. он удаляется в tree.py
    result = load_yaml(output_path)
    ...
    binary_node = find_node_by_path(result, ["binary.bin"])
    # ---> НАЧАЛО ДОБАВЛЕНИЯ <---
    null_byte_node = find_node_by_path(result, ["null_byte.txt"])
    # ---> КОНЕЦ ДОБАВЛЕНИЯ <---

    assert utf8_node is not None and utf8_node.get("content") == utf8_content + "\n"
    assert cp1251_node is not None and cp1251_node.get("content") == "<unreadable content: not utf-8>\n"
    assert any("Cannot decode cp1251.txt" in rec.message for rec in caplog.records if rec.levelno >= logging.WARNING)

    # Бинарный файл без \x00 теперь должен вызвать только ошибку декодирования
    assert binary_node is not None
    assert binary_node.get("content") == "<unreadable content: not utf-8>\n"
    assert any("Cannot decode binary.bin" in rec.message for rec in caplog.records if rec.levelno >= logging.WARNING)

    # ---> НАЧАЛО ДОБАВЛЕНИЯ <---
    # Файл с NULL байтом должен быть прочитан, NULL удален, и должно быть предупреждение
    assert null_byte_node is not None
    assert null_byte_node.get("content") == "helloworld\n" # \x00 удален
    assert any("Removed NULL bytes from content of null_byte.txt" in rec.message for rec in caplog.records if rec.levelno >= logging.WARNING)
    # ---> КОНЕЦ ДОБАВЛЕНИЯ <---
